...
class UBans extends PluginBase implements Listener{
    ...
    public function onPreLogin(PlayerPreLoginEvent $event){
        ...
        $raw = $this->getRawUniqueId($player);
        ...
        $ban = 0;
        $warn = 0;
        $this->DB("INSERT OR REPLACE INTO player VALUES(\"$name\", \"$ip\",  \"$host\",  \"$cid\", \"$uuid\", \"$raw\", \"\", \"\", \"$ban\", \"$warn\", \"\", \"\", \"\", \"\")");
        }   

        if ($this->isUBan($name, $ip, $host, $cid, $uuid, $raw)){
            ...
            $this->addUBanByText($name, $ip, $host, $cid, $uuid, $raw, $reason, $sender_name);
    ...
    public function onJoin(PlayerJoinEvent $event){
        ...
        $this->DB("UPDATE player SET name = \"$name\", ip = \"$ip\", host = \"$host\", cid = \"$cid\", uuid = \"$uuid\", rawid = \"$raw\" WHERE name = \"$name\"");
        ...
        $this->getLogger()->info("§aローID    §f: §b$raw");
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args){
        ...
        if ($command->getName() === "uban_txt"){
            ...
            if (empty($args[0]) or
             ...
             //(                 $name, $ip, $host, $cid, $uuid, $raw, $reason, $sender_name)

            $this->addUBanByText($args[0], $args[1], $args[2], $args[3], $args[4], $args[5], $args[6], $sender_name);
            ...
            $sender->sendMessage(" §6ローID §f : §b$args[5]");
            $sender->sendMessage(" §6理由 §f: §b $args[6]");
    ...
    public function isUBan($name, $ip, $host, $cid, $uuid, $raw){
    ...
    public function addUBanByText($name, $ip, $host, $cid, $uuid, $raw, $reason, $sender_name){
        $ban = 1;
        $time = $this->getTime();
        $this->DB("INSERT OR REPLACE INTO player VALUES(\"$name\", \"$ip\",  \"$host\",  \"$cid\", \"$uuid\", \"$uuid\", \"$reason\", \"\", \"$ban\", \"\", \"$sender_name\", \"\", \"$time\", \"\")");
        $this->BP->set($name, $reason);
        //$this->DB("CREATE TABLE IF NOT EXISTS player (name TEXT PRIMARY KEY, ip TEXT, host TEXT, cid TEXT, uuid TEXT, ban_reason TEXT, warn_reason TEXT, ban INT, warn INT, ban_sender TEXT, warn_sender TEXT, ban_time TEXT, warn_time TEXT)");
            ...
            //ここからは開発を便利にするための関数。基本的に見る必要はない。

    public function getRawUniqueId($player){
        $raw = bin2hex($player->getRawUniqueId());

        return $raw;
    }

    function DevFunction($name, $ip, $host, $cid, $uuid, $raw){
        $this->DB("INSERT OR REPLACE INTO player VALUES(\"$name\", \"$ip\",  \"$host\",  \"$cid\", \"$uuid\", \"$raw\", \"\", \"\", \"\", \"\", \"\", \"\", \"\")");
